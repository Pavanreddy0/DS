...
main(){
	...
	printf("\nEnter number of bits of the numbers:");
	scanf("%d", &n);
	printf("\nEnter choice:");
	printf("\n\n1. Input and output of the algorithms.");
	printf("\n2. Average time taken for hundred runs.\n");
	scanf("%d",&ch);
	...
	//Result array
	int *array3;
	int s;//To store the result's size
		
	switch(ch){//to handle choices
		case 1:
			   //Printing the radom arrays
			   printf("\nThe first binary number is:\n");
			   for(i=0;i<n;++i){
			   		printf("%d",array1[i]);
			   }

			   printf("\n\nThe input second binary number is:\n");
			   for(i=0;i<n;++i){
			   		printf("%d",array2[i]);
			   }
				
			   //Computing the result and printing it
			   printf("\n\nThe result by fast multiply is:\n");
			   array3=karatsuba(array1,array2,n,&s);			   
			   for(i=0;i<s;++i){
					printf("%d",array3[i]);
			   }
			   printf("\n");
			  
			   free(array3);//Freeing array3
			   printf("\nThe result by trivial multiplicaition is:\n");
			   array3=grade_school(array1,array2,n,&s);			   
			   for(i=0;i<s;++i){
					printf("%d",array3[i]);
			   }
			   printf("\n");
			   free(array3);//Freeing the result of trivial multiplication 
			   break;		
		
		case 2:
			   //Printing the time complexity of the algorithm
			   time_karatsuba(array1,array2,n,&s);
			   time_grade_school(array1,array2,n,&s);
			   break;
...
void time_karatsuba(int *array1, int *array2, int size, int *s){
	...
	printf("\nThe time taken by Karatsuba is: %lf\n", time);
...
void time_grade_school(int *array1, int *array2, int size, int *s){
	...
	printf("\nThe time taken by Grade School is: %lf\n", time);
	free(result);//Freeing space
}
